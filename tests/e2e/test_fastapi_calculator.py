...
def _post_json(
    ...
    """
    Wrapper around requests.post with a timeout and nicer error/skip handling.

    On Timeout or connection issues we SKIP the E2E test instead of hanging
    forever on a local dev machine. On CI (where the app is healthy) this
    won't trigger.
    """
    try:
        return requests.post(url, json=payload, headers=headers, timeout=timeout)
    except req_exc.Timeout as exc:
        pytest.skip(f"Request to {url} timed out; skipping E2E test. ({exc})")
    except req_exc.RequestException as exc:
        pytest.skip(
            f"Request to {url} timed out or connection failed: {exc}; skipping E2E test."
        )
...
def _get_json(
    ...
    try:
        ...
        pytest.skip(f"GET {url} timed out; skipping E2E test. ({exc})")
    except req_exc.RequestException as exc:
        pytest.skip(
            f"GET {url} connection failed: {exc}; skipping E2E test."
        )
...
def _put_json(
    ...
    try:
        ...
        pytest.skip(f"PUT {url} timed out; skipping E2E test. ({exc})")
    except req_exc.RequestException as exc:
        pytest.skip(
            f"PUT {url} connection failed: {exc}; skipping E2E test."
        )
...
def _delete(
    ...
    try:
        ...
        pytest.skip(f"DELETE {url} timed out; skipping E2E test. ({exc})")
    except req_exc.RequestException as exc:
        pytest.skip(
            f"DELETE {url} connection failed: {exc}; skipping E2E test."
        )
...
def register_and_login(base_url: str, user_data: dict) -> dict:
    ...
    Uses the helper to avoid hangs.
    """
...
def base_url(fastapi_server: str) -> str:
    ...
    On CI this is typically http://127.0.0.1:8000. Locally, the fastapi_server
    fixture may spin up a temporary port.
    """
...
def test_user_registration(base_url: str):
    """
    Basic happy-path registration test with timeouts and randomized user data.
    """
    ...
    assert data["email"].startswith("alice.smith.")
...
def test_user_login(base_url: str):
    ...
    for field, expected_type in required_fields.items():
        ...
        assert isinstance(
            login_data[field], expected_type
...
def test_create_calculation_addition(base_url: str):
    ...
    assert response.status_code == 201, (
        f"Addition calculation creation failed: {response.status_code} {response.text}"
    )
    data = response.json()
...
def test_create_calculation_subtraction(base_url: str):
    ...
    response = _post_json(url, payload, headers=headers)
    assert response.status_code == 201, (
        f"Subtraction calculation creation failed: {response.status_code} {response.text}"
    )
    data = response.json()
...
def test_create_calculation_multiplication(base_url: str):
    ...
    response = _post_json(url, payload, headers=headers)
    assert response.status_code == 201, (
        f"Multiplication calculation creation failed: {response.status_code} {response.text}"
    )
    data = response.json()
...
def test_create_calculation_division(base_url: str):
    ...
    response = _post_json(url, payload, headers=headers)
    assert response.status_code == 201, (
        f"Division calculation creation failed: {response.status_code} {response.text}"
    )
    data = response.json()
...
def test_list_get_update_delete_calculation(base_url: str):
    ...
    create_response = _post_json(create_url, payload, headers=headers)
    assert create_response.status_code == 201, (
        f"Calculation creation failed: {create_response.status_code} {create_response.text}"
    )
    ...
    list_response = _get_json(list_url, headers=headers)
    assert list_response.status_code == 200, (
        f"List calculations failed: {list_response.status_code} {list_response.text}"
    )
    calc_list = list_response.json()
    assert any(c["id"] == calc_id for c in calc_list), (
        "Created calculation not found in list"
    )

    # Get calculation by ID
    get_url = f"{base_url}/calculations/{calc_id}"
    get_response = _get_json(get_url, headers=headers)
    assert get_response.status_code == 200, (
        f"Get calculation failed: {get_response.status_code} {get_response.text}"
    )
    ...
    update_response = _put_json(update_url, update_payload, headers=headers)
    assert update_response.status_code == 200, (
        f"Update calculation failed: {update_response.status_code} {update_response.text}"
    )
    updated_calc = update_response.json()
    expected_result = 30  # 5 * 6
    ...
    delete_response = _delete(delete_url, headers=headers)
    assert delete_response.status_code == 204, (
        f"Delete calculation failed: {delete_response.status_code} {delete_response.text}"
    )

    # Verify deletion: GET should return 404
    get_response_after_delete = _get_json(get_url, headers=headers)
